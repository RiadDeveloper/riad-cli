...
import 'package:Riad_cli/src/commands/deps/sync.dart';
import 'package:Riad_cli/src/commands/migrate/old_config/old_config.dart'
    as old;
...
class MigrateCommand extends Command<int> {
  ...
  String get description =>
      'Migrates extension projects built with Riad v1 to Riad v2';
  ...
  Future<int> run() async {
    ...
    if (srcFile == null) {
      _lgr
        ...
        ..log(
            '${'help  '.green()} Make sure that the name of the main source file matches with `name` field in Riad.yml: `${oldConfig.name}`');
      return 1;
    }
    ...
    _lgr.startTask('Updating config file (Riad.yml)');
  ...
  void _editSourceFile(File srcFile, old.OldConfig oldConfig) {
    ...
    final description = oldConfig.description.isNotEmpty
        ...
        : 'Extension component for ${oldConfig.name}. Built with <3 and Riad.';
    final annotation = '''
