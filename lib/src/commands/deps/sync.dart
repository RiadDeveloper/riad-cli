...
import 'package:Riad_cli/src/resolver/resolver.dart';
...
const ai2RuntimeCoord =
    'io.github.RiadDeveloper.Riad:runtime:$ai2RuntimeVersion';
...
class SyncSubCommand extends Command<int> {
  ...
  Future<int> run({String title = 'Initializing'}) async {
    ...
    if (config == null && !onlyDevDeps) {
      _lgr.warn('Not in a Riad project, only dev-dependencies will be synced.');
    }
    ...
    // Exit if this is not a Riad project.
    if (config == null) {
      return 0;
    }
