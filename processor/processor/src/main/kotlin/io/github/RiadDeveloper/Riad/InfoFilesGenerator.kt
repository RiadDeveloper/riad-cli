...
import io.github.shreyashsaitwal.Riad.model.RiadYaml
...
class InfoFilesGenerator(private val extensions: List<ExtensionModel>, private val elementUtils: Elements) {
    private val projectRoot = System.getenv("Riad_PROJECT_ROOT")
    private val rawBuildDir = Paths.get(projectRoot, ".Riad", "build", "raw").apply {
    ...
    fun generateBuildInfoJson() {
        ...
        val RiadVersion = System.getenv("Riad_VERSION")

        for (ext in extensions) {
            val extJsonObj = JSONObject()
                .put("RiadVersion", RiadVersion)
                .put("type", ext.fqcn)
     ...
     * Get metadata file
     *
     * @return The Riad.yml file's data
     * @throws IOException If the input can't be read for some reason.
     */
    private fun metadataFile(): RiadYaml {
        val RiadYml = if (Paths.get(projectRoot, "Riad.yml").exists()) {
            Paths.get(projectRoot, "Riad.yml").toFile()
        } else {
            Paths.get(projectRoot, "Riad.yaml").toFile()
        }

        return Yaml.default.decodeFromStream(RiadYaml.serializer(), FileInputStream(RiadYml))
    }
